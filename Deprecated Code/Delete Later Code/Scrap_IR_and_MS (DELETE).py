...
def scrap_data(cas_id, params, data_dir, save_iter, combined_data, nist_url="https://webbook.nist.gov/cgi/cbook.cgi"):
    ...
    if params['Type'] == 'IR':
        for i in range(3):
            url = f"{nist_url}?ID={cas_id}&Units=SI&Type=IR-SPEC&Index={i}#IR-SPEC"
        ...
        full_url = requests.Request('GET', nist_url, params=params).prepare().url
...
def save_progress(progress_file, cas_id):
    try:
        with open(progress_file, 'a') as file:
            yaml.dump([cas_id], file, default_flow_style=False)
        logging.info(f'Successfully saved progress at CAS ID {cas_id}')
    except Exception as e:
...
def load_progress(progress_file):
    if os.path.exists(progress_file):
        with open(progress_file, 'r') as file:
            ...
            if data:
                return data[-1]  # Return the last processed CAS ID
    return None
...
def main():
    ...
    parser.add_argument('--save_dir', default='/Users/rudrasondhi/Desktop/Specto 0.2/Specto-0.2/Data/Mass and IR_2', help="Directory path to store scrapped data")
    ...
    last_processed_cas_id = load_progress(progress_file)
    if last_processed_cas_id:
        try:
            last_index = cas_dict.index(last_processed_cas_id) + 1
        except ValueError:
            last_index = 0
    else:
        last_index = 0

    for cas_id in cas_dict[last_index:]:
        if args.scrap_MS:
            params = {'Type': 'Mass'}
        ...
        save_progress(progress_file, cas_id)

        if counter % 2000 == 0 and combined_data:
